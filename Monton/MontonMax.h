...
#ifndef MONTON_MONTONMAX_H
#define MONTON_MONTONMAX_H
...
template <typename T, bool(*criterio)(T,T) = Max>
class MontonMax {
public:
     ...
     */
    MontonMax(const MontonMax<T,criterio> &M);
     ...
     */
    MontonMax<T, criterio> &operator=(const MontonMax<T, criterio> &M);
...
MontonMax<T, criterio>::MontonMax(): ultimo(-1), capacidad(0), elemento(NULL) {

}

template<typename T, bool (*criterio)(T, T) >
MontonMax<T, criterio>::~MontonMax() {
...
template<typename T, bool (*criterio)(T, T) >
MontonMax<T, criterio>::MontonMax(const MontonMax<T,criterio> &M): ultimo(-1), capacidad(0), elemento(NULL) {
    *this = M;
}

template<typename T, bool (*criterio)(T, T) >
MontonMax<T, criterio> &MontonMax<T, criterio>::operator=(const MontonMax<T,criterio> &M) {
...
template<typename T, bool (*criterio)(T, T) >
void MontonMax<T, criterio>::Agregar(T valor) {
...
template<typename T, bool (*criterio)(T, T) >
void MontonMax<T, criterio>::Eliminar() {
...
template<typename T, bool (*criterio)(T, T) >
int MontonMax<T, criterio>::ObtenerMin() {
    int min = elemento[0];
...
template<typename T, bool (*criterio)(T, T) >
bool MontonMax<T, criterio>::EstaVacio() {
    return ultimo == -1;
}

template<typename T, bool (*criterio)(T, T) >
void MontonMax<T, criterio>::Vaciar() {
    ultimo = -1;
...
template<typename T, bool (*criterio)(T, T)>
int MontonMax<T, criterio>::ObtenerTam() {
    return ultimo+1;
}

template<typename T, bool (*criterio)(T, T) >
void MontonMax<T, criterio>::Imprimir() {
    for(int i = 0; i < ObtenerTam(); i++){
        cout << "[" << elemento[i] << "], ";
    }
    cout << "\b\b ";
...
template<typename T, bool (*criterio)(T, T) >
void MontonMax<T, criterio>::Redimensionar() {
    if(capacidad == 0){
        ++capacidad;
    } else{
...
template<typename T, bool (*criterio)(T, T) >
void MontonMax<T, criterio>::EmpujarArriba(int indice) {
...
template<typename T, bool (*criterio)(T, T)>
void MontonMax<T, criterio>::EmpujarAbajo(int indice) {
...
template<typename T, bool (*criterio)(T, T) >
void MontonMax<T, criterio>::intercambia(T &a, T &b) {
    T aux = a;
    a = b;
    b = aux;
}
